    ...
    <div className="login-page min-h-screen flex items-center justify-center px-4 relative z-10">
      <div className="max-w-md w-full glass-card p-8">
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-cyan-400/70 mb-4">Wired and Weird</p>
          ...
          <div className="mb-6 glass-card-light p-4">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                <svg
                  className={`w-5 h-5 ${isListening ? "text-red-400 animate-pulse" : "text-gray-500"}`}
                  fill="currentColor"
                  viewBox="0 0 24 24"
                >
                ...
                {isListening && (
                  <span className="text-xs bg-red-500/20 text-red-400 px-2 py-1 rounded-full animate-pulse border border-red-500/30">
                    Listening...
                  </span>
                )}
              </div>
              <button
                ...
                className={`px-4 py-2 rounded-lg text-sm font-medium transition-all duration-200 ${
                  isListening ? "neon-btn-danger" : "neon-btn"
                }`}
              >
            ...
            {/* Transcript Display */}
            {transcript && (
              <div className="glass-card-light rounded-lg p-3 text-sm text-gray-300">
                <span className="neon-text font-medium">You said:</span>{" "}
                {transcript}
              </div>
            )}
            ...
            {lastCommand && (
              <div className="rounded-lg p-3 text-sm text-emerald-300 bg-emerald-500/10 border border-emerald-500/20">
                <span className="font-medium">✓ Command detected:</span>{" "}
                {lastCommand === "enrollment"
                  ? "Opening enrollment..."
                  : "Starting login..."}
              </div>
            )}
            ...
            {!transcript && !lastCommand && (
              <div className="text-xs text-gray-400 space-y-1">
                <p className="font-medium neon-text">Try saying:</p>
        ...
        <div className="space-y-4">
          <button
            ...
            className="w-full flex flex-col items-center justify-center py-6 px-4 border border-cyan-500/30 rounded-2xl text-sm font-medium text-white transition-all duration-300 neon-btn hover:scale-[1.02]"
          >
            <svg
          ...
          <div className="glass-card-light p-4 text-sm text-cyan-300/80">
            <p className="font-medium mb-1 text-cyan-300">How it works:</p>
        ...
        <div className="mt-8 text-center border-t border-cyan-500/10 pt-6">
          ...
          <button
            ...
            className="neon-btn-outline inline-flex items-center text-sm"
          >
            <svg
