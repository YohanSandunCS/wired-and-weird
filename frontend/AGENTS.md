# Description
This repo contains code for next.js frontend application (console used by hospital staff). This is for a one day robotics competetion to build a robot which navigates hospital corridors.

# Architecture
- There is a centralized python server
- Robot connects to it using websockets and registers it self with server
- Next.js frontend app also connects to the server as well using web sockets
...
# Changes related to communications between components
When you made changes to websocket or any other communication between different components. Please clearly mention them in `comm_changelog.md` file. So other developers can read it and implement changes in their components too.

# Other changes
Please mention any important changes in the `changelog.md` file. That you think might be important to note down.
