...
export default function BatteryStatus({ 
  ...
  if (battery === undefined) {
    return (
      <div className={`inline-flex items-center space-x-1 ${
        ...
        } border border-gray-300 rounded-sm bg-gray-100 relative`}>
          <div className={`${
            size === 'sm' ? 'w-0.5 h-1' : size === 'lg' ? 'w-1 h-2' : 'w-0.5 h-1.5'
          } bg-gray-300 absolute -right-0.5 top-1/2 transform -translate-y-1/2 rounded-r-sm`} />
        </div>
  ...
  const getBatteryTextColor = (level: number) => {
    if (level <= 15) return 'text-red-700'
    if (level <= 30) return 'text-orange-700'
    if (level <= 50) return 'text-yellow-700'
  ...
  return (
    <div className={`inline-flex items-center space-x-2 ${
      ...
      <div className={`relative ${
        size === 'sm' ? 'w-4 h-2' : size === 'lg' ? 'w-8 h-4' : 'w-6 h-3'
      } border-2 border-gray-400 rounded-sm bg-white ${isStale ? 'opacity-50' : ''}`}>
        {/* Battery Fill */}
        <div 
        ...
        <div className={`${
          size === 'sm' ? 'w-0.5 h-1' : size === 'lg' ? 'w-1 h-2' : 'w-0.5 h-1.5'
        } bg-gray-400 absolute -right-0.5 top-1/2 transform -translate-y-1/2 rounded-r-sm`} />
      </div>
      
      {/* Battery Percentage */}
      {showPercentage && (
        <span className={`font-medium ${textColor} ${isStale ? 'opacity-50' : ''}`}>
          {battery}%
        </span>
      )}
